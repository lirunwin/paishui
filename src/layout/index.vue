...
export default class Layout extends Vue {
  ...
  changeVal(val) {
    ...
    if (
      val.path.indexOf("dashboard") === 1 ||
      val.path.indexOf("map") === 1 ||
      val.path.indexOf("hydrualics") === 1
    )
    ...
    if (val.path.indexOf("dashboard") === 1 && this.sidebar)
      this.$store.dispatch("app/toggleSideBarShow", false);
  ...
  mounted() {
    ...
    if (path.indexOf("dashboard") === 1 && this.$store.state.app.sidebar.show)
      this.$store.dispatch("app/toggleSideBarShow", false);
    else if (
      path.indexOf("dashboard") === -1 &&
      !this.$store.state.app.sidebar.show
    )
