<template>
  <div id="tags-view-container" class="tags-view-container">
    ...
    <div class="right_box">
      ...
      <i class="el-icon-more-outline more-list-trigger" @mouseenter="toggleMore('trigger')">
        <transition name="fade">
          <div v-if="moreListShow && visitedViews.more.length !== 0" class="more-list" @mouseleave="toggleMore('container')">
            <template>
              <router-link v-for="tag in visitedViews.more" :key="tag.path" ref="tag" :class="isActive(tag)?'active':''" :to="{ path: tag.path, query: tag.query, fullPath: tag.fullPath }" tag="span" class="more-view-item" @click.middle.native="!isAffix(tag)?closeSelectedTag(tag):''" @contextmenu.prevent.native="openMenu(tag,$event)">
...
export default {
  ...
  watch: {
    $route() {
      ...
      this.$store.dispatch('routeSetting/routesChangeByTagsView', this.$route.name)
    },
    visible(value) {
      if (value) {
  ...
  methods: {
    ...
    getTagsNum() {
      ...
      that.$nextTick(() => {
        ...
        var tagsWidth = that.$refs.scrollPane.$el.clientWidth - 100
        var maxTagNum = parseInt(tagsWidth / 150)
        var tagsItems = that.$refs.scrollPane.$el.getElementsByClassName('tags-view-item')
        ...
        var nullNum = 0
        if (tagsWidth > tagsItemWidth) { nullNum = parseInt((tagsWidth - tagsItemWidth) / 120) }
