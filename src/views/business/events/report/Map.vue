...
export default class MapView extends Vue {
  ...
  async initMap() {
    ...
    const { initCenter, initZoom } = this.$store.getters.appconfig
    ...
    this.view.on('singleclick', (e) => {
      if (this.enableDeviceSelect) {
        ...
        this.spaceQuery(e.coordinate)
      }
      if (this.enableCoordinateSelect) {
        this.coordinateChange(e.coordinate)
        const pointLon = e.coordinate[0].toFixed(6) //经度
        const pointLat = e.coordinate[1].toFixed(6) //纬度
