      ...
      <div class="content-info">
        <div class="pieChartArea">
          <div class="pItem" v-for="item in staList" :key="item.title">
            <div class="pchart">
              <decorationChart :color="item.color" :chartData="item" :fontSize="$listeners.fontSize" />
            </div>
          ...
          <div class="bItem" v-for="item in orderList" :key="item.title">
            ...
            <div class="bcomb">
              <div class="sta">
                ...
                <div class="staWrap">
                  完成率：
                  <div class="staPercent">{{ item.percent }}%</div>
                </div>
              </div>
              <div class="cha">
...
export default {
  ...
  data() {
    return {
      ...
      value: '全部',
      staList: [
        { title: '巡检里程', code: 'length', value: 0, unit: 'km', color: '#05BCD7' },
        { title: '巡检任务数', code: 'taskNum', value: 0, unit: '个', color: '#FFB54C' },
        { title: '工单总数', code: '', value: 0, unit: '个', color: '#0EA7FF' }
      ],
      orderList: [
        { title: '汛情工单总数', value: 0, unit: '个', percent:0, img: require('./images/汛情.png') },
        { title: '事件工单总数', value: 0, unit: '个', percent: 0, img: require('./images/事件.png') },
        { title: '隐患工单总数', value: 0, unit: '个', percent: 0, img: require('./images/维修.png')}
      ],
      reportList: [
        { title: '隐患上报', normal: { title: '一般', value: 0 }, warning: { title: '严重', value: 0 } },
        { title: '汛情上报', normal: { title: '普通', value: 0 }, warning: { title: '警情', value: 0 } },
        { title: '事件上报', normal: { title: '普通', value: 0 }, warning: { title: '紧急', value: 0 } }
      ]
    }
  },
  mounted() {
  ...
  methods: {
    getPageData() {
      ...
      getRequestResult({ blockCode: 'patrolCount' }).then((res) => {
          ...
          getRequestResult({ blockCode: 'maintainCount' }).then((res) => {
            let index=this.orderList.findIndex(item=>item.title==='隐患工单总数')
            this.orderList[index].value=res.sumNum
            this.orderList[index].percent=res.accomplishRate
          })
        }).then(()=>{
          getRequestResult({ blockCode: 'reportCount' }).then((res) => {
            let index=this.reportList.findIndex(item=>item.title==='事件上报')
            this.reportList[index].normal.value=res[0].ordinaryNum
            this.reportList[index].warning.value=res[0].severityNum
          })
        }).then(()=>{
          getRequestResult({ blockCode: 'troubleCount' }).then((res) => {
            let index=this.reportList.findIndex(item=>item.title==='隐患上报')
            this.reportList[index].normal.value=res.filter(item=>item.level==='一般')[0].num
            this.reportList[index].warning.value=res.filter(item=>item.level==='严重')[0].num
          })
        })
    }
  }
  ...
  .content-info {
    ...
    .barChartArea {
      ...
      .bItem {
        ...
        height: 33%;
        display: flex;
        margin: 0 0 0.052083rem /* 10/192 */ 0;
        .bchart {
          flex: 0.2;
        }
        .bcomb {
          ...
          .sta {
            flex: 0.5;
            padding: 0.026042rem /* 5/192 */ 0.104167rem /* 20/192 */;
            display: flex;
