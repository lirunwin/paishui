    ...
    <el-row type="flex" justify="space-around">
      <el-col :span="12">
        ...
        <el-form-item style="margin-bottom: 15px" label="角色复制">
          <el-select v-model="copyRole" multiple size="small" placeholder="请选择需要复制的用户角色"
            filterable clearable collapse-tags 
            @focus="getCopyDeptUserList" @change="copyRoleChange" >
...
export default {
    ...
    return {
      ...
      rules: {
        username: [
          ...
          { pattern: regUserName(), message: '登录名只能输入英文、数字', trigger: 'blur' }
        ],
        password: [
  ...
  methods: {
    ...
    beforeAvatarUpload(file, type) {
      ...
      console.log(this.electronicFile)
      const _this = this
      const reader = new FileReader()
      // 转base64
      reader.onload = function(e) {
        type === 'sign' ? _this.form.signPic = e.target.result : _this.form.avatar = e.target.result // 将图片路径赋值给src
      }
    ...
    // 编辑参数回显
    handleEdit() {
      this.ruleForm = this.assignment(this.ruleForm, this.data)
