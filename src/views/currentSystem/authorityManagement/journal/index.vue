  ...
  <div class="app-container">
    <div class="hander_journal">
      ...
      <el-date-picker v-model="problems.startTime" style="width:140px;"  size="small" type="date" placeholder="请选择开始时间" :picker-options="startOptions" value-format="yyyy-MM-dd"/> ~
        ...
        <el-select v-if="!btnFlag" v-model="userids" multiple size="small" 
          style="display: inline-block; width: 200px;margin-right:10px" placeholder="请选择需要查询的人员"
      ...
      startOptions:{//开始时间控制
			...
			endOptions:{//结束时间控制
					...
					if(this.problems.startTime){
							return  new Date(this.problems.startTime)-1000*60*60*24>time.getTime();
					}
					},
			},
  ...
  methods: {
    ...
    getDeptLogType(){
      ...
      let data = {
        ...
        endDate: this.postSearch.endDate,
      }
    ...
    arrayGroupDept(aryList){
      ...
      for (let i = 0; i < aryList.length; i++) {
        ...
        if (!map[ai.deptId]) {
            ...
            map[ai.deptId].push({type:ai.type,typeName:ai.typeName,num:ai.num})
            map[ai.deptId].deptName = ai.deptName;
        }
      }
      let res = []
