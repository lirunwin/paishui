...
export default class Journal extends Vue {
  ...
  endOptions = {
    ...
    disabledDate: (time) => {
      if (this.problems.startTime) {
        return (
          new Date(this.problems.startTime).getTime() - 1000 * 60 * 60 * 24 >
          time.getTime()
        );
      }
    },
  ...
  arrayGroupDept(aryList) {
    ...
    for (let i = 0; i < aryList.length; i++) {
      ...
      if (!map[ai.deptId]) {
        map[ai.deptId] = [
          { type: ai.type, typeName: ai.typeName, num: ai.num },
        ];
  ...
  cellDblclick(row, column) {
    ...
    if (this.btnFlag) {
      ...
      if (
        this.postSearch.columnLabel === "order" ||
        this.postSearch.columnLabel === "deptname"
      ) {
