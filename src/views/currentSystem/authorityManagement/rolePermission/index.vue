  ...
  <div class="app-container">
    ...
    <div class="handerRole">
      ...
      <div class="btn_box">
        ...
        <el-button :disabled="multipleSelection.length===0" type="primary" size="small" @click="handleDelete">删除角色</el-button>
    ...
    <main>
      ...
      <aside>
        <el-form ref="form" :model="detailed" label-width="120px" :disabled="disabledEdit">
          <el-form-item label="角色名称：">
            <el-input v-model="detailed.roleName" style="width: 300px" placeholder="请输入角色名称" type="text" />
          </el-form-item>
          <el-form-item label="备注：" style="margin: 20px 0">
            <el-input v-model="detailed.roleDesc" style="width: 300px" placeholder="备注说明" type="textarea" />
          </el-form-item>
          <el-form-item label="系统功能：">
            <div class="sys-list">
              <el-tree ref="outTree" :data="treeData" show-checkbox node-key="id" :props="defaultProps" :expand-on-click-node="false" :render-content="renderContent" @node-expand="changeCss" />
            </div>
          </el-form-item>
        </el-form>
      </aside>
    ...
    <el-dialog v-if="dialogVisible" :title="dialogTitle" :visible.sync="dialogVisible" width="500px" top="10px">
      <div class="doalog-content">
        <input-item type="text" placeholder="请输入角色名称" title="角色名称：" nominate="roleName" custom-style="display: inline-block; width: 80%; line-height: 32px;" :value="detailed.roleName" :max-length="50" :disabled="false" :required="true" @changeValue="onInput" />
        <Textarea
          type="textarea"
          placeholder="备注说明"
          title="备注："
...
import Textarea from '@/components/FormItem/Textarea'
...
export default {
    ...
    multipleSelection(value) {
      ...
      if (this.multipleSelection.length === 1) {
        ...
        existMenus.forEach(item => {
          const node = this.$refs.outTree.getNode(item)
          if (node !== null && node.isLeaf) {
            this.$refs.outTree.setChecked(node, true)
          }
        })
  ...
  methods: {
      ...
      this.$nextTick(() => {
        ...
        for (const i in levelName) {
          if (levelName[i].parentNode) {
            levelName[i].parentNode.style.float = 'left'
          }
        }
      })
    },
    ...
    saveEdit() {
      ...
      const selected = this.$refs.outTree.getCheckedNodes().map(item => item.id)
      const halfSelected = this.$refs.outTree.getHalfCheckedKeys().map(item => item)
