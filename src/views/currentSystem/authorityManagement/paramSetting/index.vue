    ...
    <el-tabs v-model="activeName" @tab-click="getSavedParams">
      <template v-for="item in tabList">
        <el-tab-pane :key="item.name" :label="item.label" :name="item.name">
          <el-form label-position="right" label-width="200px" class="custom-form" ref="paramSettingForm" :model="activeName==='paramSetting' ? appFormModel : webFormModel" inline-message>
            <template v-if="activeName === 'paramSetting'">
              <template v-for="appForm in appForms">
                <el-form-item :key="appForm.prop" :label="`${appForm.label}：`" class="custom-form-item" :rules="appForm.rules" :prop="appForm.prop">
                  <el-input v-if="appForm.type === 'input'" v-model="appFormModel[appForm.prop]"></el-input>
                  <el-select v-if="appForm.type === 'select'" v-model="appFormModel[appForm.prop]" style="display: inline-block">
                    <template v-for="option in appForm.appSelects">
                      <el-option :key="option.id" :value="option.id" :label="option.name"></el-option>
                    </template>
                  </el-select>
                  <i class='el-icon-info' style="font-size: 16px;color: gray;" :title="appForm.title?appForm.title:''"></i>
                </el-form-item>
              </template>
            </template>
            <template v-else>
              <template v-for="webForm in webForms">
                <el-form-item :key="webForm.prop" :label="`${webForm.label}：`" class="custom-form-item" :rules="webForm.rules" :prop="webForm.prop">
                  <el-input v-if="webForm.type === 'input'" v-model="webFormModel[webForm.prop]"></el-input>
                  <el-color-picker v-if="webForm.type === 'color'" v-model="webFormModel[webForm.prop]"></el-color-picker>
                  <i class='el-icon-info' style="font-size: 16px;color: gray;" :title="webForm.title?webForm.title:''"></i>
                </el-form-item>
              </template>
            </template>
            <el-form-item>
...
export default class ParamSetting extends  Vue{
  ...
  appForms= [
    {
      ...
      prop: 'peopleSpeed',
      type: 'input',
      rules: { required: true, message: '人巡最快速度不能为空！', trigger: 'blur' },
    ...
    },
    {
      ...
      prop: 'peopleMaxalw',
      type: 'input',
      rules: { required: true, message: '人巡最大到位容差不能为空！', trigger: 'blur' },
    ...
    },
    {
      ...
      prop: 'carSpeed',
      type: 'input',
      rules: { required: true, message: '车巡最快速度不能为空！', trigger: 'blur' },
    ...
    },
    {
      ...
      prop: 'carMaxalw',
      type: 'input',
      rules: { required: true, message: '车巡最大到位容差不能为空！', trigger: 'blur' },
    ...
    },
    {
      ...
      prop: 'uploadRate',
      type: 'input',
      rules: { required: true, message: '上传频率不能为空！', trigger: 'blur' },
    ...
    },
    {
      ...
      prop: 'routeMaxspeed',
      type: 'input',
      rules: { required: true, message: '轨迹最大速度不能为空！', trigger: 'blur' },
  ...
  webForms= [
    ...
    },
    {
      ...
      prop: 'routePt',
      type: 'input',
      rules: { required: true, message: '轨迹点细度不能为空！', trigger: 'blur' },
      title:"轨迹上点的大小"
    },
    {
      label: '轨迹线细度（px）',
      prop: 'routeLine',
      type: 'input',
      rules: { required: true, message: '轨迹线细度不能为空！', trigger: 'blur' },
      title:"轨迹线的粗细"
    }
  ];
  ...
  getSavedParams() {
    getParams().then(res => {        
      const {id, peopleSpeed, peopleMaxalw, carSpeed, carMaxalw, uploadRate, routeMaxspeed, ismark, 
      plineColor, pointColor, lineColor, routePt, routeLine, routePtnum } = res.result.records[0];
