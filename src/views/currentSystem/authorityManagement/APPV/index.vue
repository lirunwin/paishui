<template>
  <div class="app-container">
    <div style="line-height: 54px;">
      ...
      <el-date-picker v-model="publishTime.startTime" style="width:140px;" size="small" type="date" placeholder="请选择开始时间" :picker-options="startOptions" value-format="yyyy-MM-dd" /> ~
      <el-date-picker v-model="publishTime.endTime" style="width:140px;" size="small" type="date" placeholder="请选择结束时间" :picker-options="endOptions" value-format="yyyy-MM-dd" />
      ...
      <el-button type="primary" size="small" :disabled="publishDisabled" @click="handleClick('publish')">发布</el-button>
    </div>
    ...
    <el-dialog :visible.sync="detailDialog" title="详细信息查看" width="900px">
      <div class="detail-container">
        <el-row class="info-row">
          <el-col class="info-col" :span="12">
            <span>类型：</span><span>{{ detailInfo.appType === '1' ? ' 一体化采集' : '综合业务' }}</span>
          </el-col>
          <el-col class="info-col" :span="12">
            <span>版本号：</span><span>{{ detailInfo.versionsub }}</span>
          </el-col>
        </el-row>
        <el-row class="info-row">
          <el-col class="info-col" :span="12">
    ...
    <el-dialog :visible.sync="uploadDialog" title="选取文件" width="400px" :before-close="canClose" :close-on-click-modal="false">
      <div v-loading="uploadWaiting" style="width: 100%">
        <el-form ref="appForm" :model="appForm" label-width="90px" class="demo-ruleForm">
          <el-row type="flex" justify="space-around">
            <el-col :span="24">
              <el-form-item style="margin-bottom: 15px" label="app类型">
              ...
              <el-form-item style="margin-bottom: 15px" label="附件上传">
                ...
                >
                  <div style="width: 100%; display: flex; justify-content: space-around">
                    ...
                    <el-button v-if="fileList.length !== 0" style="margin-left: 20px" size="small" type="success" @click.stop="upload">上传文件</el-button>
                  </div>
                </el-upload>
              </el-form-item>
            </el-col>
...
export default {
  ...
  data() {
    return {
      ...
      endOptions: {// 结束时间控制
        disabledDate: time => {
          if (this.publishTime.startTime) {
            return new Date(this.publishTime.startTime) - 1000 * 60 * 60 * 24 > time.getTime()
          }
        }
      },
      ...
      column: [
        {
          ...
          formatter: (row, col, cellValue, index) => {
            if (cellValue === '1') return '一体化采集'
            if (cellValue === '2') return '综合业务'
          }
        },
        {
          label: '版本号',
          prop: 'versionsub', // version
          width: '150'
        },
        {
          label: '上传包文件',
      ...
      appForm: {
        appType: '2' // app类型 1 一体化采集；2 综合业务
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    getRecords() {
      ...
      if (!((!that.publishTime.startTime && !that.publishTime.endTime) || (that.publishTime.startTime && that.publishTime.endTime))) {
    ...
    verificationFileType(data) {
      if (data.raw.type === 'text/plain' || data.raw.type === 'application/vnd.android.package-archive') {
        this.fileList.push(data.raw)
      } else {
        this.$message({
          message: '只能上传apk和txt文件',
          type: 'error'
        })
        return
      }
    ...
    upload() {
      ...
      // 增加类型
      form.append('type', this.appForm.appType)

      for (const item of this.fileList) {
        if (item.type === 'text/plain') form.append('txtFile', item)
        else if (item.type === 'application/vnd.android.package-archive') form.append('appFile', item)
      }
      // 判断附件是否存在
      if (!form.get('txtFile')) {
        this.$message({
