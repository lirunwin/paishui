...
import { getOneFactoryWaterQualityYL, getOneFactoryWaterQualityZD } from '@/api/dashboard'
...
export default {
  ...
  created() {
    this.getOneFactoryWaterQualityYLList()
    this.getOneFactoryWaterQualityZDList()
  },
  methods: {
    drwaChart() {
      const options = [
     ...
     */
    getOneFactoryWaterQualityYLList() {
      var that = this
      getOneFactoryWaterQualityYL({}).then((res) => {
        if (res.code !== -1) {
     ...
     */
    getOneFactoryWaterQualityZDList() {
      var that = this
      getOneFactoryWaterQualityZD({}).then((res) => {
        if (res.code !== -1) {
          let _resultList = res.result
          _resultList.forEach(function(item, index) {
            that.ntuData.push({
