...
import { getTwoFactoryTakingWaterSupply } from '@/api/dashboard'
export default {
  props: ['name'],
  data() {
    return {
  ...
  created(){
    this.getTwoFactoryTakingWaterSupplyList()
  },
  methods: {
    drwaChart() {
    ...
    /**
     * @description 二厂（取供水曲线图）
     */
    getTwoFactoryTakingWaterSupplyList(){
      var that = this
      let date = new Date();
      ...
      getTwoFactoryTakingWaterSupply({}).then(res => {
         if(res.code !== -1) {
