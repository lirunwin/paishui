<template>
  ...
  <div style="padding: 0 8px;overflow-y:auto;" class="i-scrollbar">    
    ...
    <tf-legend class="legend_dept" label="查询条件" isopen="true" style="clear: both" title="选择字段、属性及构造语句组合成查询过滤的语句，通过构建的条件过滤数据。">    
      ...
      <el-button size="mini" type="primary" style="width:40%;display:inline-block;" @click="queryResult" :disabled="analysisDisable">
        <i v-if="analysisDisable" class="el-icon-loading"/>查询</el-button>
      <el-button size="mini" type="primary" style="width:40%;display:inline-block;margin-top:8px;" @click="clearAll">清空</el-button>
    </tf-legend>
    ...
    */
    ...
    rowC: function (row) {
      ...
      this.$store.dispatch('map/changeMethod', {
        ...
        widgetid: 'HalfPanel',
        label: '更多信息',
        param: { data: rowData, colsData, rootPage: this }
      })
    },
    gotoGeometry (geometry) {
        this.queryLayer && this.queryLayer.getSource().addFeature(new Feature({ geometry }))
        let extent = geometry.getExtent()
        this.mapView.getView().animate({ center: [(extent[0] + extent[2]) / 2, (extent[1] + extent[3]) / 2]}, { zoom: 20 })
    }
  },
  destroyed() {
    this.clearAll()
