  ...
  <div style="width: 100%; height: 100%; padding:8px;">
    ...
    <div v-loading="mainLoading" style="width:100%;height:calc(100% - 46px);margin-top:8px;">
      ...
      <el-table id="repairRecordTable" class="mapTable" :data="tableData" border stripe height="calc(100% - 80px)" @row-dblclick="showMissionInfo">
        ...
        <el-table-column :formatter="formatter" prop="typeName" :show-overflow-tooltip="true" label="维修类型" align="center" />
        ...
        <el-table-column :formatter="formatter" prop="khbh" :show-overflow-tooltip="true" label="客户编号" align="center" />
        <el-table-column :formatter="formatter" prop="khxm" :show-overflow-tooltip="true" label="客户名称" align="center" />
        <el-table-column :formatter="formatter" prop="khdz" :show-overflow-tooltip="true" label="客户地址" header-align="center" align="left" />
        <el-table-column :formatter="formatter" prop="khdz" :show-overflow-tooltip="true" label="联系电话" header-align="center" align="left" />
        ...
        <el-table-column :formatter="formatter" prop="disposeNote" :show-overflow-tooltip="true" label="处理说明" header-align="center" align="left" />
...
import { repairRecords, exportRepairRecords, repairTypeStatistic } from '../api/reportFormApis.js'
import { esriConfig } from 'staticPub/config'
import { IP } from '@/utils/request'
import { loadModules } from 'esri-loader'
export default {
  name: 'WxWeiXiuJiLuTaiZhang',
  ...
  methods: {
    ...
    */
    ...
    exportList() {
      ...
      exportRepairRecords(data).then(res => {
        var reader = new FileReader()
        reader.readAsDataURL(res)
        reader.onload = (e) => {
          const a = document.createElement('a')
          a.download = '维修记录台账数据表（' + this.date_select.value.map(e =>
    ...
    getList() {
      ...
      repairRecords(data).then(res => {
        res = res.result
        this.tableData = res.records
        this.total = res.total
        this.title = '维修记录台账数据表（' + this.date_select.value.map(e =>
          e.split(' ')[0].split('-')).map(e =>
