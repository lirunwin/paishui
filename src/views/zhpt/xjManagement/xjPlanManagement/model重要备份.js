        ...
        return {
            ...
            endOptions:{//结束时间的限定
                disabledDate:time=> {
                    ...
                    return  new Date(this.searchModel.input.startTime)-1000*60*60*24>time.getTime();
                    }
                },
            },
            searchModel:{//查询模块
    ...
    methods: {
        ...
        */
        taskArrangeSubmit() {
            ...
            if((((!tempValue.planBegindate||!tempValue.planEnddate)&&tempValue.period!=3)||(tempValue.period==3&&(!tempValue.planBegindate2)))&&title==""){
                title='请选择巡检时间！';
            }
            ...
            if (input.period == 1) { //一天一巡
                ...
                input.planEnddatePick.disabledDate=time=>{ 
                    ...
                    if(input.planBegindate){
                        let greaterThanStart=(new Date(input.planBegindate)-1000*60*60*24>time.getTime());//大于等于开始时间
                ...
                input.planBegindatePick.disabledDate=time=>{
                    ...
                    if(input.planEnddate){
                        let lessThanEnd=new Date(input.planEnddate)-1000*60*60*24>time.getTime();//小于等于最后一天
                        return  isMonDay||greaterThanNew||!lessThanEnd
                    }else{
                        return isMonDay||greaterThanNew
                    }
                }
                input.planEnddatePick.disabledDate=time=>{
                    let greaterThanNew=(time.getTime() < Date.now() - 8.64e7);//大于等于当前
                    ...
                    if(input.planBegindate){
                        let greaterThanStart=(new Date(input.planBegindate)-1000*60*60*24>time.getTime());//大于等于开始时间
                        return greaterThanNew||isSunDay||greaterThanStart;
                    }else{
                        return greaterThanNew||isSunDay;
                    }
                }
                ...
                //只能选择当天及以后并且大于等于结束时间
                input.planEnddatePick.disabledDate=time=>{ 
                    let greaterThanNew=(time.getTime() < Date.now() + 8.64e7*parseInt(input.periodDay - 2));//大于等于当前
                    if(input.planBegindate){
                        let greaterThanStart=(new Date(input.planBegindate)+1000*60*60*24>time.getTime());//大于等于开始时间
                        return  greaterThanStart||greaterThanNew;
                    }else{
