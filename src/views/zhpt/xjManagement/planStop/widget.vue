<template>
    <div style="width: 100%; height: 100%; padding:8px;">
        <el-row>
            ...
            <el-button size="small" type="primary" :disabled="buttonControl.canPause" @click="checkPausePage()">暂停</el-button>
        </el-row>
        <el-table class="mapTable" :data="searchModel.getData.allRow" border style="width: 100%;margin-top: 8px;" ref="table1" height="calc(100% - 88px)" stripe highlight-current-row @select="searchTableSelect" 
        ...
        <el-row style="margin-top: 8px;width:350px;">
            <el-col :span="20">
                <el-pagination small background layout="total, sizes, prev, pager, next" :current-page="searchModel.pageInfo.current" :page-size="searchModel.pageInfo.size" :page-sizes="searchModel.pageInfo.pageSizes" 
                @size-change="changeSize"
                @current-change="changeCurrent"
                @prev-click="changeCurrent"
                @next-click="changeCurrent"  :total="searchModel.getData.tableTotal" />
            </el-col>
        </el-row>
            ...
            <div style="width:100%;padding-right:8px;">
                <tf-table-legend label="巡检信息" isopen="true" style="margin-top: 8px;">
                    <div style="width:100%; display: flex; flex-wrap: wrap;">
                        <div class="flexDiv" ref="xjType"><span class="flexTitle" >巡查类型：</span><div class="flexInfo"></div></div>
                        <div class="flexDiv" ref="xjGroup">
                            <span class="flexTitle" >巡查组：</span><div class="flexInfo"></div>
                        </div>
                        <div class="flexDiv" ref="xjWorker">
                            <span class="flexTitle">巡查人：</span><div class="flexInfo"></div>
                        </div>
                        <div class="flexDiv"  ref="xjBeginAndEndTime">
                            <span class="flexTitle">起止时间：</span><div class="flexInfo"></div>
                        </div>
                        <div class="flexDiv"  ref="xjPlanNumber">
                            <span class="flexTitle">计划完成率：</span><div class="flexInfo"></div>
                        </div>
                    </div>
                </tf-table-legend>
                                ...
                                <template  v-for="(item,index) in planModel.input.planTypeInfo">
                                    <el-tab-pane  :key="index+''" :name="index+''" :label="item.layerName">
                                        <div style="width:100%; height: 200px;">   
                                            <el-table  v-show="item.planInfo.isCheckData"  :data="item.planInfo.tableData" :ref="'datatable'+index" style="width: 100%;margin-top: 8px;" height="calc(100% - 40px)" stripe highlight-current-row
                                            @row-click="rowSelectPlan">                 
                                                ...
                                                <el-table-column type="index" width="50" label="序号" />
                                                <el-table-column v-for="(value, index2) in item.planInfo.header" :key="index2" :label="value.label" align="center" :prop="value.value" 
                                                show-overflow-tooltip/>
                                            </el-table>
                                            <el-table v-show="!item.planInfo.isCheckData"  :data="item.planInfo.pageInfo.currentData"  style="width: 100%;margin-top: 8px;" height="calc(100% - 40px)" stripe highlight-current-row
                                            @row-click="rowSelectPlan">
                                                <template slot="empty">
                                                <img src="@/assets/icon/null.png" alt="">
                                                <p class="empty-p">暂无数据</p>
                                                ...
                                                <el-row style="margin-top: 8px;width:100%;">
                                                    <el-col :span="20">
                                                        <template v-if="item.planInfo.isCheckData">
                                                            <el-pagination v-show="item.planInfo.tableData&&item.planInfo.tableData.length>0" small background layout="total"  
                                                            :total="item.planInfo.total"  />
                                                        </template>
                                                        <template v-else> 
                                                            <el-pagination v-show="item.planInfo.tableData&&item.planInfo.tableData.length>0"   small background 
        ...
        <el-dialog v-dialogDrag title="暂停审核" :visible.sync="diaVisiableStop" width="520px" top="calc(50vh - 336px)" @close="cancelSubmit"  append-to-body>
            <div style="width:100%;padding-right:8px;">
                <div style="width:100%; display: flex; flex-wrap: wrap;">
                    <div class="flexOne" >
                        <span class="flexTitle" >申请人：</span>
                        <div class="flexInfo">
                            <el-input type='text' v-model="applyInfo.submitUserName"  disabled size="small"></el-input>
                        </div>
                    </div>
                    <div class="flexOne" >
                        <span class="flexTitle">申请说明：</span>
                        <div class="flexInfo">
                            <el-input type="textarea" :rows="2"  disabled size="small" v-model="applyInfo.applyNote"></el-input>
                        </div>
                    </div>
                    <div class="flexOne" >
                        <span class="flexTitle">开始时间：</span>
                        <div class="flexInfo">
                            <el-input type='text' v-model="applyInfo.applyStarttime"  disabled size="small"></el-input>
                        </div>
                    </div>
                    <div class="flexOne" >
                        <span class="flexTitle">结束时间：</span>
                        <div class="flexInfo">
                            <el-input type='text' v-model="applyInfo.applyEndtime"  disabled size="small"></el-input>
                        </div>
                    </div>
                    <div class="flexOne" >
                        <span class="flexTitle">提交时间：</span>
                        <div class="flexInfo">
                            <el-input type='text' v-model="applyInfo.createTime"  disabled size="small"></el-input>
                        </div>
                    </div>
                    <div class="flexOne" >
                        <span class="flexTitle">审核意见：</span>
                        <div class="flexInfo">
                            <el-select v-model="applyInfo.auditResult" style="width:100%;" size="small" placeholder="请选择巡检审核意见" >
                    ...
                    <div class="flexOne" >
                        ...
                        <div class="flexInfo">
                            <el-input type="textarea" :rows="2" v-model="applyInfo.auditNotes" size="small"></el-input>
                        </div>
                    </div>
                </div>
            </div>
            <span slot="footer" class="dialog-footer">
                ...
                <el-button  type="primary" size="small" @click="stopUpdataSubmit()">确定</el-button>
            </span>
        </el-dialog>
                    ...
                    <template v-for="item in auditInfoHisList">
                        <div class='itemList' :key='item.id' :value="item.createTime" style='width:100%; border: 1px solid #EBEEF5; padding: 5px;'>
                            <div class='baseInfo' style='width:100%;'>
                                <div>
                                    <span style="width: 250px; display: inline-block;">申请时间：{{item.createTime}}</span>
                                    <span  style="width: 250px; display: inline-block;">申请人：{{item.submitUserName}}</span>
                                </div>
                                <div style="padding-top:20px">
                                    <span  style="width: 250px; display: inline-block;">计划开始时间：{{item.applyStarttime}}</span>
                                    <span  style="width: 250px; display: inline-block;">计划结束时间：{{item.applyEndtime}}</span>
                                </div>   
                                <div style="padding-top:20px">
                                    <span>申请说明：{{item.applyNote}}</span>
                                </div>    
                            </div>
                            <div class='auditInfoPart' style='width:100%;'>
                                ...
                                <div v-else>
                                    <div style="padding-top:20px">
                                        <span style="width: 250px; display: inline-block;">审核时间：{{item.auditTime}}</span>
                                        <span style="width: 250px; display: inline-block;">审核人：{{item.auditUserName}}</span>
                                        <span style="width: 250px; display: inline-block;">审核意见：{{item.auditResult}}</span>
                                    </div>
                                    <div style="padding-top:20px">
                                        <span>审核说明：{{item.auditNote}}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default class xjPlanStop extends Vue {
    @Prop() data:any;
    searchModel={//查询模块
            ...
            buttonControl={//控制按钮的启用与禁用
                canPause:true,//控制废除按钮，初始化默认禁用状态
            };
            arcgis={//arcgis相关的对象
    ...
    destroyed(){
        ...
        */
        ...
        searchTableSelect(e, s) {
            ...
            if(e.length ==1&&e[0].planState == '申请暂停'){
                this.buttonControl.canPause = false //可暂停
            }else{
                this.buttonControl.canPause = true //不可暂停
            }
        }
        ...
        */
        checkPausePage() {
            ...
            stopList(data).then(res => {
                this.diaVisiableStop=true;
        ...
        */
        stopUpdataSubmit() {
            let dto = {
                ...
                "auditNotes": this.applyInfo.auditNotes,
                "auditResult": this.applyInfo.auditResult,
                "auditId":this.$store.state.user.userId
                }
            let templateServiceType = '06' //上报类型 0代表类型为年计划
