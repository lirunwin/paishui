<template>
  <div style="width: 100%; height: 100%; padding:8px;">
    <el-row>
      ...
      <el-date-picker
        v-model="searchModel.input.timeForMission"
        type="datetimerange"
        size="small"
        style="width:300px"
        range-separator="至"
      ...
      <el-button size="small" type="primary" :disabled="buttonControl.canPause" @click="checkPausePage()"
        >暂停</el-button
      >
    </el-row>
    <el-table
    ...
    <el-row style="margin-top: 8px;width:350px;">
      <el-col :span="20">
        <el-pagination
          ...
          :page-size="searchModel.pageInfo.size"
          :page-sizes="searchModel.pageInfo.pageSizes"
      ...
      <div style="width:100%;padding-right:8px;">
        <div style="width:100%; display: flex; flex-wrap: wrap;">
          ...
          <div class="flexOne">
            ...
            <div class="flexInfo">
              <el-select
                v-model="applyInfo.auditResult"
                style="width:100%;"
                size="small"
                placeholder="请选择巡检审核意见"
              >
        ...
        <div style="width:100%; display: flex; flex-wrap: wrap;">
          <template v-for="item in auditInfoHisList">
            <div
              ...
              style="width:100%; border: 1px solid #EBEEF5; padding: 5px;"
            >
...
export default class xjPlanStop extends Vue {
  ...
  searchModel = {
    //查询模块
    input: {
      //多选获取的输入是数组，但后端获取的是字符串，多个值使用逗号进行隔开，故这里将输入和实际发送请求的数据分开
      timeForMission: '', //巡查日期
      chooseGroup: '', //部门
      chooseWorker: '', //人员
      planType: '', //巡检类型
  ...
  getGroupUserMap() {
    ...
    getGroupUserMap(data).then((res) => {
      ...
      res.result.groupUserVoList.forEach((item) => {
        let index = planWorkerAndGroup.findIndex((item2) => {
          return item2.id == item.groupId
        })
        ...
        if (index == -1) {
          planWorkerAndGroup.push({
            index: planWorkerAndGroup.length + 0,
            id: item.groupId,
            name: item.groupName,
            data: [{ id: item.userId, name: item.userIdName }]
          })
          searchWorkerAndGroup.push({
            index: planWorkerAndGroup.length + 0,
            id: item.groupId,
            name: item.groupName,
            data: [{ id: '', name: '全部' }, { id: item.userId, name: item.userIdName }]
          })
        } else {
      ...
      if (!this.hasGroup) {
        ...
        let sIndex = searchWorkerAndGroup.findIndex((item) => {
          return item.id == this.searchModel.input.chooseGroup
        })
        let aIndex = planWorkerAndGroup.findIndex((item) => {
          return item.id == this.planModel.input.chooseGroup
        })
  ...
  getData() {
    ...
    let data = {
      ...
      planEnddate: this.searchModel.input.timeForMission[1]
        ? this.searchModel.input.timeForMission[1].split(' ')[0] + ' 23:59:59'
        : ''
    }
  ...
  initUpDateData(data) {
    ...
    inputData.planType = data.inspectType.split(',').map((item) => {
    ...
    //dom更新完毕后
    this.$nextTick(() => {
      inputData.chooseWorker = data.inspectUserList.split(',').map((item) => {
        return parseInt(item)
      }) //人员获取完成后，初始赋值
    })
  }
  ...
  addMap() {
    ...
    ).then(([MapView, GeometryService, LengthsParameters, geometryEngine, Polygon]) => {
  ...
  planInfo(e) {
    ...
    this.$nextTick(() => {
      ...
      this.$refs.xjGroup.innerHTML =
        '<span class="flexTitle">巡查组：</span>' +
        '<span class="flexInfo">' +
        (e.inspectGroup == '' ? '全部' : e.inspectGroup) +
        '</span>'
      //@ts-ignore
      ...
      ;(this.$refs.xjBeginAndEndTime.innerHTML =
        ...
        (this.$refs.xjPlanNumber.innerHTML =
          '<span class="flexTitle">计划完成率：</span>' + '<span class="flexInfo">' + e.planPercent + '%</span>')
      //@ts-ignore
  ...
  toggleSelection(saveData) {
    ...
    let rows = []
    planInfo.tableData.forEach((item) => {
      let index = indexList.findIndex((item2) => {
  ...
  xjTypeChange(e?) {
    ...
    if (planType) {
      ...
      typeList.forEach((item) => {
        let index = typeInfo.findIndex((item2) => {
          return item2.typeId == item
        })
  ...
  initData(header, isCheckData, isServer) {
    ...
    return {
      ...
      sidExist: {
        //查询的管网设备中必须保证所有的设备都有sid，不存在sid则不能下派计划
        exist: true, //默认都存在
        layerName: '' //不存在sid的图层
      }
    }
  ...
  groupGetInfoBySid(data) {
    let that = this
    let index = this.planModel.updatePlan.findIndex((item) => {
      return item.typeId == data.typeId
    })
    let searchData = this.planModel.updatePlan[index]
    searchData.data = searchData.data.map((item) => {
  ...
  stopUpdataSubmit() {
    ...
    checkData(dto, templateServiceType)
      .then((res) => {
        this.diaVisiableStop = false
        this.getData()
      })
