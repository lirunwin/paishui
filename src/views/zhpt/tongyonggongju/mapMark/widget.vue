      ...
      <el-row style="margin-top: 8px">
        <el-col :span="20">
          <el-pagination
            ...
            :total="total"
            @current-change="listRefersh"
            @size-change="listRefersh"
          />
        </el-col>
      </el-row>
      <el-row style="margin-top: 8px">
...
export default {
  ...
  methods: {
    ...
    deleteMark(row) {
      this.$confirm(`是否删除 ${row.markName} 书签？`, '提示', {
    ...
    deleteMarks() {
      ...
      this.$confirm('确定删除选中的 ' + selects.length + '条书签？', '提示', {
        distinguishCancelAndClose: true,
        confirmButtonText: '确定',
        cancelButtonText: '取消'
      }).then(() => {
