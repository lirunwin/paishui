<template>
  ...
  <div class="panel-container i-scrollbar">
    ...
    <div class="op-box" v-show="drawType === 'point'">
      <el-form>
        ...
        <el-form-item label="管线编号" style="margin-top:10px">
          <el-input size="small" v-model="selectPipeID" style="width:100%" disabled> </el-input>
        </el-form-item>
      </el-form>
    </div>
    ...
    <!-- 分析结果 -->
    <div class="op-box">
      <el-button type="primary" size="small" style="width:100%" :loading="analysisStatus" @click="analysis"
        >开始分析</el-button
      >
    </div>
    <div class="op-box">
...
export default {
  ...
  methods: {
    ...
    select() {
      ...
      this.drawer = new iDraw(this.map, this.drawType, {
        endDrawCallBack: (fea) => {
          fea = new GeoJSON().readFeature(
            turf.buffer(turf.point(fea.getGeometry().getCoordinates()), 0.5 / 1000, { units: 'kilometers' })
          )
          this.getAnalysisPipe(fea).then((featuresArr) => {
            if (featuresArr.length !== 0) {
              ...
              this.queryFeature.setStyle(
                comSymbol.getDrawStyle(2, 'f00', 3, '#66B1FF', 'rgba(255,255,255,0.3)', [15, 15])
              )
