    ...
    <div class="result-box">
      <div class="item-head">
        ...
        <el-tooltip
          class="item"
          effect="dark"
          content="选择一段管线，判断该段管线的埋设是否达到标准的最小覆土深度"
          placement="top"
        >
    ...
    >
      ...
      <el-table-column prop="start" width="65" align="center" label="起点">
        <template slot-scope="scope">
          <p :style="{ color: !scope.row.start ? 'red' : 'black' }">
            {{ scope.row.start ? '合格' : '不合格' }}
          </p></template
        >
      </el-table-column>
      <el-table-column prop="end" width="65" align="center" label="终点">
        <template slot-scope="scope">
          <p :style="{ color: !scope.row.end ? 'red' : 'black' }">{{ scope.row.end ? '合格' : '不合格' }}</p></template
        >
      </el-table-column>
    </el-table>
    <!-- 图表 -->
...
export default {
  ...
  methods: {
      ...
      this.drawer = new iDraw(this.mapView, this.drawType, {
        ...
        endDrawCallBack: (fea) => {
          ...
          if (this.drawType === 'point') {
            ...
            let geometryJson = turf.buffer(turf.point(fea.getGeometry().getCoordinates()), tolerateDis / 1000, {
              units: 'kilometers'
            })
    ...
    showRes(row) {
      ...
      // 定位
      let feature = this.vectorLayer
        .getSource()
        .getFeatures()
        .find((fea) => fea.get('LNO') && fea.get('LNO') === row.id)
      if (feature) {
    ...
    getPipeSoilDepth(feature) {
      ...
      if (typeName == '移动线缆' || typeName == '电信线缆' || typeName == '广电线缆') feaType = '电信线缆'
      else {
        feaType = typeName
      }
      ...
      let xAxis = [
        0,
        getLength(new GeoJSON().readFeature(feature).getGeometry(), { projection: proj, radius: 6378137 }).toFixed(2)
      ]
