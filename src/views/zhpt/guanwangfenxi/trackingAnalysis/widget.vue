<template>
  ...
  <div class="panel-container i-scrollbar">
    ...
    <div class="result-box">
      <div class="item-head">
        ...
        <el-tooltip
          class="item"
          effect="dark"
          content="选择一段管线，追踪该管线的起点方向和止点方向相连的管线"
          placement="top"
        >
      ...
      <div class="result-description">
        ...
        <div v-cloak style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;">
          {{ selectPipelineInfo.id }}
        </div>
      </div>
    </div>
    <div class="op-box">
      <div class="item-head" style="margin-top:0">
        追踪距离
    ...
    <div class="op-box">
      <el-button type="primary" size="small" style="width:100%" @click="doQuery()" :loading="doLoading"
        >追踪分析</el-button
      >
    </div>
    <!-- 表格 -->
        ...
        >
          ...
          <el-table-column label="结果" align="center">
            <template slot-scope="scope"
              ><span>结果{{ scope.$index + 1 }}</span></template
            >
          </el-table-column>
          <el-table-column label="管段数量" align="center">
            <template slot-scope="scope"
              ><span>{{ scope.row.features.features.length }}个</span></template
            >
          </el-table-column>
          <el-table-column label="管点数量" align="center">
            <template slot-scope="scope"
              ><span>{{ scope.row.facilities.features.features.length }}个</span></template
            >
          </el-table-column>
  ...
  methods: {
      ...
      this.drawer = new iDraw(this.map, 'point', {
        endDrawCallBack: (drawFea) => {
          let fea = new GeoJSON().readFeature(
            turf.buffer(turf.point(drawFea.getGeometry().getCoordinates()), 0.3 / 1000, { units: 'kilometers' })
          )
    ...
    queryPipesParams(edges) {
      ...
      let sqlQueryParam = new SuperMap.GetFeaturesBySQLParameters({
        ...
        datasetNames: [mapConfig.iServerUrl.pipelineDataServer.dataSource + ':' + this.selectFeature.get('TYPENAME')],
    ...
    queryPoint(pipeFeatures) {
      ...
      let sqlQueryParam = new SuperMap.GetFeaturesBySQLParameters({
        toIndex: -1,
        maxFeatures: 10000000,
        datasetNames: [mapConfig.iServerUrl.pipelineDataServer.dataSource + ':' + this.selectFeature.get('attachName')],
    ...
    expendRowData(row) {
      ...
      features.forEach((feature) => {
        ...
        if (subtype) {
          if (facilityTypeObj.hasOwnProperty(subtype)) facilityTypeObj[subtype].push(feature)
          else facilityTypeObj[subtype] = [feature]
        } else {
          if (adjunct) {
            if (facilityTypeObj.hasOwnProperty(adjunct)) facilityTypeObj[adjunct].push(feature)
            else facilityTypeObj[adjunct] = [feature]
          } else {
            if (facilityTypeObj.hasOwnProperty('其他')) facilityTypeObj['其他'].push(feature)
            else facilityTypeObj['其他'] = [feature]
          }
        }
      })
      let yAxisData = []
