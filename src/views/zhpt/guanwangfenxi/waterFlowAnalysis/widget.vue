    ...
    <div id="Legend" class="Legend">
      <div class="label" @click="openstate = !openstate">
        ...
        <el-tooltip
          ...
          placement="right"
        >
          <i ref="info" class="el-icon-info" /> </el-tooltip
        ><el-checkbox v-model="ractSelect" style="margin-left: 4px;">地图显示</el-checkbox>
      </div>
        ...
        endDrawCallBack: (drawFea) => {
          ...
          this.getAnalysisPipe(fea).then((resObj) => {
            ...
            if (resObj) {
              ...
              let centerPoint = new Feature({
                geometry: new Point([(lineCoors[0][0] + lineCoors[1][0]) / 2, (lineCoors[0][1] + lineCoors[1][1]) / 2])
              })
      ...
      new iNetAnalysis().findPath(points[0], points[1]).then((res) => {
        if (res) {
          if (res.result.pathList.length !== 0) {
            ...
            this.this.layerData = [{ name: '给水管线', value: pathFeatures.length, pathFeatures }]
          }
