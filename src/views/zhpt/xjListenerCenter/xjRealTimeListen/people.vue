...
<div class='peoplePage' v-loading='loading'>
  <el-row>
    ...
    <el-select v-model="currentUser" collapse-tags multiple filterable clearable class="searchInput" size="small" placeholder="请选择人员" @change="getShowUserList">
      <el-option v-for="item of userList" :key="'currentUser_'+item.signUser" :label="item.signUserName" :value="item.signUser" />
    </el-select>
            ...
            <div class='userBaseInfo'>
              <div class='peopleState' :class='item.peopleBackground'>
                <div class='image'>
                  ...
                  <template v-else>
                    <el-avatar :size="32"><i style="font-size:18px" class="el-icon-s-custom"></i> </el-avatar>
                  </template>
                </div>
                <span style="font-size:12px">{{item.signType}}</span>
              </div>
              <div class='userName' :title='"点击定位"' @click='goToPoint(item)'>{{item.signUserName}}</div>
              <div class='clickTool' style="float:right;">
                ...
                <el-tooltip effect="dark" content="设备使用情况" placement="top-start">
                  <el-button size="mini" style="padding:4px;" @click="showWay(item,showContent.equipment)">
                    <span class='icon iconfont iconxunchariji'></span>
              ...
              <template v-if="item.pagePathInfo&&item.pageShowContent==showContent.path">
                <ul class='pathInfoDiv'>
                  <li v-for="(pathitem, index) in item.pagePathInfo.splitPath" :key="'pathInfoDivLi_'+index">
                    ...
                    <span class="name">{{ pathitem.timeFrom + ' - ' + pathitem.timeTo.split(' ')[1] }}</span>
                  </li>
...
import {getUserSignList,getSection,getEquipmentInfo,getGPSAll,getPlanTimeIntervalPage } from '@/api/xjApi'
...
export default {
  ...
  methods:{
    ...
    getUserList(){
      this.loading=true;
      getUserSignList({deptId:this.currentDepart,time:tool.getDayEndTime(this.currentDate)}).then(res=>{
        const tempList=res.result.pdasignVoList||[];
    ...
    getShowUserList(){
      ...
      for(let item of this.userList){
        //当选取人员后，人员必须在选择范围内
        if(this.currentUser.length>0&&!this.currentUser.includes(item.signUser)){
          continue;
        }
      ...
      this.mapShowPeople(tempUserList);
    },
    ...
    mapShowPeople(peopelList){
      ...
      this.$emit("addPoint",{type:'people',data:showList});
    },
    ...
    getEquipmentInfo(data){
      ...
      getEquipmentInfo(params).then(res=>{
        data.pageEquipmentInfo.infoList = res.result.records||[];
        data.pageEquipmentInfo.total=res.result.total;
      })
