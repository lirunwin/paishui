  ...
  <el-tabs v-model="activeName" ref="tabs" v-loading="mainLoading">
    <el-tab-pane :label="'人员\n(' + worker + ')'" name="worker">
      <el-row style="margin-top: 8px;">
        ...
        <el-select
          v-model="department"
          style="display: inline-block; margin-left: 3px;width:calc(100% - 63px);margin-bottom: 4px;"
        ...
        <el-select
          v-model="group"
          style="display: inline-block; margin-left: 3px;width:calc(50% - 55px);"
          size="small"
          placeholder="请选择"
          @change="loadStute"
        >
        ...
        <el-select
          v-model="status"
          style="display: inline-block; margin-left: 3px;width:calc(50% - 60px);"
          size="small"
          placeholder="请选择"
          @change="loadStute"
        >
        ...
        <template v-if="workerTable.length > 0">
          ...
          >
            <div style="display:flex;justify-content:space-between;align-items:center; ">
              ...
              <el-link target="_blank" :title="item.signUserName" class="nameList" @click="jump(item)">{{
                item.signUserName
              }}</el-link>
    ...
    <el-tab-pane :label="'隐患(' + totalWarring + ')'" name="warring">
      ...
      >
        <li
          v-for="(item, index) in fixTable"
          :key="index"
          style="width:100%;padding: 5px;border: 1px solid #dadada; padding: 10px 5px;margin-top:4px;height:80px;"
        >
          <div style="display:flex;justify-content:space-between;align-items:center;height:100%;">
            <div :ref="'troubleImg/' + index"></div>
            <!-- <el-image style="width: 80px; height: 60px" :src="item.src[0]" fit="cover"></el-image> -->
            <div
              style="display:inline-block;position: relative;width: calc(100% - 100px);height:80%;margin-left: 10px;"
            >
              <el-link style="position: absolute;letter-spacing: 1px;" type="primary" @click="jump(item)">{{
                item.typeName || '未知类型'
              }}</el-link>
              <el-tooltip
                v-if="(item.address || '').length > 9"
      ...
      <el-row style="margin-top: 8px">
        <el-pagination
          ref="paginationWarring"
          small
          background
          layout="total, sizes, prev, next"
          :page-sizes="[5, 10, 50, 100]"
          :total="totalWarring"
          @current-change="loadWarring"
          @size-change="loadWarring"
          style=" width: 250px; display: inline-block;"
        />
    ...
    <el-tab-pane :label="'工地(' + placeTotal + ')'" name="place">
      ...
      >
        <li
          v-for="(item, index) in placeTable"
          :key="index"
          style="width:100%;padding: 5px;border: 1px solid #dadada; padding: 10px 5px;margin-top:4px;height:80px;"
        >
          <div style="display:flex;justify-content:space-between;align-items:center;height:100%;">
      ...
      <el-row style="margin-top: 8px">
        <el-pagination
          ...
          :page-sizes="[5, 10, 50, 100]"
          :total="placeTotal"
          @current-change="loadPlace"
          @size-change="loadPlace"
          style=" width: 250px; display: inline-block;"
        />
...
export default class XjRealTimeListen extends Vue {
  ...
  async init() {
    ...
    var done = () => {
      if (++recoad == 4) this.mainLoading = false
    }
    //@ts-ignore
  ...
  loadRoad(isChangePage?) {
    ...
    var url =
      (this.tableType == 'log'
        ? '/gps/gps/page?startDate=' + startDate + '&endDate=' + endDate + '&userId='
        : '/gps/plan/getPlanTimeIntervalPage?planEnddate=' +
          this.setTime +
          ' 23:59:59' +
          '&planBegindate=' +
          this.setTime +
    ...
    request({ url: url, method: 'get' }).then((res: any) => {
      ...
      if (res.code == 1) {
        ...
        ;(this.$refs.pageTableLength as HTMLElement).innerHTML =
          pages.currentPage + '/' + Math.ceil(res.total / pages.currentPage) + '页'
