		...
		<el-dialog v-dialogDrag :title="addTitle" :visible.sync="diaVisibleAdd" width="550px">
			<el-form :model="addForm" :rules="addRules" ref="addForm" label-width="120px">
				...
				<el-form-item label="图层名称：" prop="layerName" style="margin-top: 15px">
					<el-select v-model="addForm.layerName" size="small" placeholder="请选择图层" style="width: 350px;">
						<el-option v-for="item in layerNames" :key="item.value" :label="item.label" :value="item.label"></el-option>
					</el-select>
				</el-form-item>
...
export default {
	...
	mounted() {
		...
		success: (data) => {
			...
			this.layerNames = [{value: 999, label: '巡检点'}, {value: 1000, label: '巡检线'},{value: 1001, label: '隐患点'},{value: 1002, label: '工地监护点'}]
			for(let i=0,ii=data.length;i<ii;i++){
				var layer = data[i];
				if(equipmentLayerId.indexOf(layer.id) > 0){
					this.layerNames.push({ value: layer.id, label: layer.name })
				}
			}
		},
			...
			this.$refs[formName].validate((valid) => {
				if (valid) {
					let addInfo = {
						...
						layerName: this.addForm.layerName, //图层名称
