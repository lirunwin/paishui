<template>
  <div id="troubleSearchReport" class="troubleSearchReport">
    ...
    <el-date-picker v-model="querySearch.endTime"   size="small" type="date" placeholder="请选择结束时间" :picker-options="endOptions" value-format="yyyy-MM-dd"/>      
		<el-button size="small" style="margin-left: 5px;"  type="primary" @click="getTroubleSearchAnalysisData()">查询</el-button>
	...
	<div id="pdfDom" class="pdfDom">
		...
		<div class="statistics">
			<el-table border ref="table1" stripe style="width: 100%;" height="100%"
				:data="reportData" :header-cell-style="{'text-align':'center'}">
				<template slot="empty">
				...
				<el-table-column prop="toubleRangeName"  label="隐患等级" width="180px" :show-overflow-tooltip='true' align="left" />
				...
				<el-table-column prop="notes" sortable label="排除隐患情况" width="260" :show-overflow-tooltip='true' align="left" />
	...
	<!-- <div class="pagination-area">
  	...
  	<div v-show="false">
		<el-table border stripe style="width: 100%;" height="100%"  id="materiaStatistics"
			:data="reportDataTotal" :header-cell-style="{'text-align':'center'}">
  ...
  data() {
    return {
		...
		groupData: [],   //组数据
      	userData: [],    //人员数据
		chartTitle: '',     //图表标题
		stateStatus: [			// 码表 => 隐患状态 0无需处理、1巡查、2已消除、3消除申请
			{ id:'',name:'全部' },
			{ id:'0',name:'无需处理' },
			{ id:'1',name:'巡查' },
			{ id:'3',name:'消除申请' },
  ...
  methods: {
	...
	*/
    ...
    dateDiff(){
      ...
      //图表标题
      this.chartTitle = "隐患记录台账信息表-" + bintime.getFullYear() +"年"+(bintime.getMonth()+1)+'月'+bintime.getDate()+"日~"+endtime.getFullYear() +"年"+(endtime.getMonth()+1)+'月'+endtime.getDate()+'日'
    },
    ...
    getInitData(){
			...
			let tempTime = (endDate.getFullYear() +"-"+(this.getMonthOrDay(endDate.getMonth()+1)) +"-"+ this.getMonthOrDay(endDate.getDate()));
