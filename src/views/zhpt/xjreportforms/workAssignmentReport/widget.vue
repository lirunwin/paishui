<template>
  <div id="workAssignmentReport" class="workAssignmentReport">
    <div class="menus">
      ...
      <export-btn table-id="workAssignmentReportTable" />
    </div>
...
import reportTable from '@/views/zhpt/xjreportforms/workAssignmentReport/reportTable'
import { getScheduling } from '@/api/xjApi'
...
export default {
  ...
  data() {
    return {
      dataList: {
        title: '2021.6.11日报',
        tableId: 'workAssignmentReportTable',
        data: []
      },
  ...
  mounted: function() {
    ...
    this.dataList.title = '排班记录(' + data.now + '~' + data.next + ')'
    ...
    getDailyReportData() {
      ...
      getScheduling(data).then(res => {
        if (res.code == 1) {
          ...
          this.dataList.title = '排班计划表(' + this.startTime + '~' + this.endTime + ')'
        }
        console.log(res)
      })
    },
